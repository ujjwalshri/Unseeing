...
const Post = ({ post }) => {
  ...
  const { mutate: likePost, isPending: isLiking } = useMutation({
    ...
    onSuccess: (updatedLikes) => {
      queryClient.invalidateQueries({queryKey : ['posts']});// this is not the best ux experience
  ...
  const { mutate: commentOnPost, isPending: isCommenting } = useMutation({
    ...
    onError: () => {
      toast.error("Cannot comment on post");
    },
  });
          ...
          <div className="flex gap-2 items-center">
            ...
            <span className="text-gray-300 flex gap-1 text-sm">
              <Link to={`/profile/${postOwner._id}`}>
                {isMyPost == true ? "Your post" : postOwner.branch}
              </Link>
              <span>·</span>
